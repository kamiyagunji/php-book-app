                ...
                <div class="card-body">
                    ...
                    <p class="card-subtitle mb-2 text-muted">
                        ...
                        <?= $this->Form->postLink('削除する', ['controller' => 'Answers', 'action' => 'delete', $answer->id],
                            ['confirm' => '回答を削除します。よろしいでしょうか？'], ['class' => 'card-link']) ?>
                    </p>
                </div>
            </div>
        <?php endforeach; ?>
...
<section class="answer-post mb-5">
    ...
    <?php if ($answers->count() >= 100): ?>
        <p class="text-center">回答数が上限に達してるためこれ以上回答することはできません</p>
    <?php else: ?>
        <?= $this->Form->create($newAnswer, ['url' => '/answers/add']) ?>
        <?php
            echo $this->Form->control('body', ['type' => 'textarea', 'label' => 'false', 'value' => '', 'maxLength' => 200]);
            echo $this->Form->hidden('question_id', ['value' => $question->id]);
        ?>
        <?= $this->Form->button('投稿する', ['class' => 'btn btn-warnig']) ?>
        <?= $this->Form->end() ?>
    <?php endif; ?>
</section>